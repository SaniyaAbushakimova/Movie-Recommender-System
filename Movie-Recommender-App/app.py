...
from myfuns import (get_displayed_movies, get_popular_movies,
...
sidebar = html.Div(
    [
        ...
        dbc.Nav(
            [
                dbc.NavLink("System 1 - Top 10 Rated Movies", href="/", active="exact"),
                dbc.NavLink("System 2 - ICBF", href="/system-2", active="exact"),
            ],
            vertical=True,
            pills=True,
        ),
...
def render_page_content(pathname):
    if pathname == "/":
        return html.Div(
            [
                html.H1("Top 10 Most Rated Movies with Aveage Rating > 4.3"),
                dbc.Row(
                    [
                        html.Div(
                            [
                                *[
                                    get_movie_card(movie, num_ratings=True)
...
def get_movie_card(movie, with_rating=False, num_ratings=False):
    return html.Div(
        dbc.Card(
            [
                dbc.CardImg(
                ...
                dbc.CardBody(
                    [
                        ...
                        html.Div([
                            html.P(f"Number of Ratings: {movie.RatingsNum}", className="text-center mb-0"),
                            html.P(f"Average Rating: {movie.AvgRating:.2f}", className="text-center mt-0")
                        ]) if num_ratings else None,
                    ]
                ),
            ]
            + (
...
@app.callback(
    Output("button-recommend", "enabled"),
